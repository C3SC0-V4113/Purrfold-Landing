...
interface MagicCardBaseProps extends React.AriaAttributes {
...
export function MagicCard(props: MagicCardProps) {
  ...
  const glowFrom = isOrbMode(props)
    ? (props.glowFrom ?? 'var(--color-premium-card-gradient-from)')
    : 'var(--color-premium-card-gradient-from)';
  const glowTo = isOrbMode(props)
    ? (props.glowTo ?? 'var(--color-premium-card-gradient-to)')
    : 'var(--color-premium-card-gradient-to)';
  ...
  useEffect(() => {
    ...
    const updateTheme = () => setIsDarkTheme(element.classList.contains('dark'));
    updateTheme();
    ...
    >
      ...
      {mode === 'gradient' && (
        <motion.div
          ...
          className="pointer-events-none absolute inset-px z-30 rounded-[inherit] opacity-0 transition-opacity duration-200 ease-out group-hover:opacity-100"
          style={{
            background: useMotionTemplate`
              radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
                ${gradientColor},
            ...
            `,
            opacity: shouldReduceMotion ? 0 : gradientOpacity,
          }}
        />
      )}
